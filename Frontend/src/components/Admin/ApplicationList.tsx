    ...
    const fetchData = async () => {
      try {
        const response = await axios.get<IApplication[]>(
          `${Base_URL}/admin/getapplications`
        );
  ...
  const handleViewClick = async (applicationId: string) => {
    try {
      const response = await axios.get(
        `${Base_URL}/admin/applicationview/${applicationId}`
      );
      // console.log(response,"res")
      navigate("/admin/applicationdetails", {
        state: { applicationData: response.data },
      });
    } catch (error) {
              ...
              <div className="flex overflow-hidden rounded-xl border border-[#dce0e5] bg-white">
                <table className="flex-1">
                  ...
                  <tbody>
                    {loading ? (
                      <tr>
                        <td colSpan={7} className="py-3 px-6 text-center"></td>
                      </tr>
                      ...
                      filteredApplications.map((application, index) => (
                        <tr key={index} className="border-t border-[#dce0e5]">
                          ...
                          <td className="px-4 py-3 text-[#111418] text-sm">
                            <button
                              onClick={() =>
                                handleViewClick(application.applicationId)
                              }
